...
class maingame():
    ...
    def night(self):
        self.fog=pygame.Surface((winw,tile_size*len(self.land)))
        self.fog.fill((20,20,20))
        self.light=pygame.transform.scale(pygame.image.load('maps/light.png').convert_alpha(),(tile_size*7,tile_size*7))
        self.light_rect=self.light.get_rect()
    
    def render_night(self,player):
        local=self.fog.copy()
        self.light_rect.center=player.rect.center
        local.blit(self.light, self.light_rect)
        self.win.blit(local,(0,0),special_flags=pygame.BLEND_MULT)
    ...
    def run(self):
        ...
        while True:
            ...
            self.lvl.run()
            if fog:self.render_night(self.lvl.player_grp.sprite)
...
class level():
    ...
    def score_show(self):
        coin=pygame.transform.scale(pygame.image.load("tiles/2.png").convert_alpha(),(tile_size-10,tile_size-10))
